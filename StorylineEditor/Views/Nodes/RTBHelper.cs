    ...
    public static class RTBHelper
    {
        ...
        private static void IsEditingChanged(DependencyObject obj, DependencyPropertyChangedEventArgs e)
        {
            if ((bool)e.NewValue)
            {
                ...
                {
                    using (var stringReader = new StringReader(node.Name))
                    {
        ...
        private static void RefreshInlines(RichTextBox rtb)
        {
            ...
            string newName = XamlWriter.Save(rtb.Document);
        ...
        public static string GetFlowDocumentContent(string flowDocument)
        {
            if (!string.IsNullOrEmpty(flowDocument))
            {
                using (var stringReader = new StringReader(flowDocument))
                {
                    using (var xmlTextReader = new XmlTextReader(stringReader))
                    {
                        var document = (FlowDocument)XamlReader.Load(xmlTextReader);
                        ...
                        foreach (Paragraph paragraph in document.Blocks)
                        {
                            foreach (var inline in paragraph.Inlines)
                            {
                                ...
                                result += documentRun.Text;
                            }

                            if (paragraph != document.Blocks.LastBlock) result += Environment.NewLine;
                        }
